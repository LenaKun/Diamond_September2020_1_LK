...
namespace CC.Data
{
	[MetadataType(typeof(CC.Data.MetaData.EmergencyCapMetaData))]
	public partial class EmergencyCap : IValidatableObject
	{
		public EmergencyCap()
		{
			this.CurrencyId = "USD";
		...
		public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			using (var db = new ccEntities())
			{
				...
				var duplicateName = db.EmergencyCaps.Any(f => f.Name == this.Name && f.Id != this.Id);
				if (duplicateName)
				{
					yield return new ValidationResult("Emergency Cap Name must be unique.");
				}
			}
			...
			using (var db = new ccEntities())
			{
				...
				var q = from ec in db.EmergencyCaps
