...
namespace CC.Web.Tests
{
    ...
    ///</summary>
    [TestClass()]
    public class ClientsControllerTest_GlobalOfficer
    {
        ...
        public ClientsController Target
        {
            get
            {
                if (target == null)
                {
                    target = GetTarget_ForUser(FixedRoles.GlobalOfficer, "Agency1_FirstTest");
        ...
        public void IndexDataTable_Test()
        {
            ...
            Assert.IsTrue(c1.Count() > 0, "Global Officer for First Region must get Client1");

            var c2 = param.aaData.Where(x => x.FirstName == "Client2");
            Assert.IsTrue(c2.Count()> 0, "Global Officer can get all clients");
