...
namespace CC.Web.Tests
{
    ...
    ///</summary>
    [TestClass()]
    public class ClientsControllerTest_AgencyUser
    {
        ...
        public ClientsController Target
        {
            get
            {
                if (target == null)
                {
                    target = GetTarget_ForUser(FixedRoles.AgencyUser, "Agency1_FirstTest");
        ...
        public void IndexDataTable_Test()
        {
            ...
            Assert.IsTrue(c1.Count() > 0, "User Agency for First Agency must get Client1");

            var c2 = param.aaData.Where(x => x.FirstName == "Client2");
            Assert.IsTrue(c2.Count() ==0, "User Agency for First Agency can not clients from other agency");
